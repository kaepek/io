...
export class DualShockControlInputSourceHandler extends ControlInputSourceHandler {
    ...
    /*async handle_input(input) {
        // emit state to controller
        await super.handle_input({ ...input});
    }*/
    ...
    async ready() {
        ...
        this.gamepad.ondigital = async (label, value) => super.handle_input({source:this.type, type: "button", label, value });
        this.gamepad.onanalog = async (label, value) => super.handle_input({source:this.type, type: this.input_types[label], label, value });
    }
