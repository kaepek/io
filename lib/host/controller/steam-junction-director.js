...
export class StreamJunctionDirector {
    ...
    constructor(
        ...
        // control_word_handlers
        // here need to build the control_word_handler$
        this.control_word_handlers$ = merge(...this.control_word_handlers.map((handler) => handler.$));

        this.control_source_input_new_words$ = control_source_input_router.$.pipe(
            map(this.handle_control_input),
            filter((value) => value.length != 0)
        )
        //this.control_source_input_router_subscription = control_source_input_router.$.subscribe(this.handle_control_input);
    ...
    handle_control_word_output(word) {
