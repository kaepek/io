...
class DualShockToTorquePhaseModel extends InputToModelBase {
    ...
    gamepadArgs = null;
    scale = 1;
    state = { torque: 0, phase: 127, direction: true }; //cw 0 false / ccw 1 true
    ...
    */
    ...
    async handleInput(inputObj) {
        // if (inputObj.type === "stick" && inputObj.label === "rStickX") console.log("inputObj", inputObj);
        ...
        if (inputObj.type === "stick" && inputObj.label === "rStickX") {
            this.state.phase = inputObj.value;
            if (this.state.phase > 112 && this.state.phase < 142) {
                this.state.phase = 127;
            }
            // this.state.phase = (((inputObj.value ) % phase_max) + phase_max) % phase_max;
        }
